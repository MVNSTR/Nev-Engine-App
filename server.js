...
function getCreator(account,target,targetGroupId){ if(target==='group'){ const gid=targetGroupId||account.groupId; if(!gid) throw new Error('Group ID is not set.'); return {groupId:String(gid)}; } if(!account.userId) throw new Error('User ID is not set for this account.'); return {userId:String(account.userId)}; }
...
function savePlayableNote(discordId,job,note,description){ const notes=getNotes(discordId); const id=note.groupId ? `${job.id}:${note.part}:${note.groupId}` : `${job.id}:${note.part}`; const idx=notes.findIndex(n=>n.id===id); const item={id,jobId:job.id,title:job.title,part:note.part,groupId:note.groupId||'',name:note.name,assetId:note.assetId,playbackSpeed:job.playbackSpeed,speedUp:job.speedUp,description:description||job.robloxDescription||DEFAULT_DESCRIPTION,status:'available',availableAt:note.availableAt||new Date().toISOString(),createdAt:note.createdAt||job.createdAt||new Date().toISOString(),accountName:job.robloxAccountLabel||''}; if(idx>=0) notes[idx]=item; else notes.unshift(item); saveNotes(discordId,notes); }
...
async function uploadJob(discordId,{jobId,accountId,target,assetName,description}){ const job=getJob(discordId,jobId); if(!job) throw new Error('Job not found'); if(job.status!=='done') throw new Error('Conversion is not done'); if(job.filesExpired) throw new Error('Converted files expired'); const accounts=getAccounts(discordId); const account=accounts.find(a=>a.id===accountId); if(!account) throw new Error('Account not found'); updateJob(discordId,jobId,{robloxStatus:'processing',robloxAccountId:account.id,robloxAccountLabel:account.label,robloxDescription:description||account.defaultDescription||DEFAULT_DESCRIPTION,songNotes:[]}); let destinations = [null]; if(target==='group'){ if(!account.groupId) throw new Error('Group ID is not set for this account.'); destinations = account.groupId.split(',').map(x=>x.trim()).filter(Boolean); if(destinations.length===0) throw new Error('Group ID is not set for this account.'); } const notes=[]; const outputs=(job.outputs||[]).slice().sort((a,b)=>a.part-b.part); let idx=0; const tasks=[]; for(const dest of destinations){ for(const out of outputs){ const baseName=assetName||job.title; const robloxDisplayName=outputs.length>1?`${sanitizeRobloxName(baseName)} - Part ${out.part}`:sanitizeRobloxName(baseName); let suffix=''; if(target==='group'&&destinations.length>1){ suffix=` (Group ${dest})`; } const uiNoteName=outputs.length>1?`${sanitizeRobloxName(baseName)} - Part ${out.part}${suffix}`:`${sanitizeRobloxName(baseName)}${suffix}`; const noteIndex=idx++; notes[noteIndex]={part:out.part,groupId:dest||'',name:uiNoteName,status:'queued',reason:'Antre untuk di-upload...'}; tasks.push({dest,out,uiNoteName,robloxDisplayName,noteIndex}); } } updateJob(discordId,jobId,{songNotes:notes,robloxUploads:notes}); const promises=tasks.map(t=>{ return (async()=>{ const file=path.join(OUTPUT_DIR,t.out.filename); if(!fs.existsSync(file)){ notes[t.noteIndex]={part:t.out.part,groupId:t.dest||'',name:t.uiNoteName,status:'failed',reason:'File hasil convert hilang/kedaluwarsa'}; updateJob(discordId,jobId,{songNotes:[...notes],robloxUploads:[...notes]}); return; } try{ if(target==='group'&&destinations.length>1){ addLog(discordId,jobId,`Uploading Part ${t.out.part} to Roblox Group ${t.dest}...`); } else { addLog(discordId,jobId,`Uploading Part ${t.out.part} to Roblox...`); } notes[t.noteIndex].status='processing'; notes[t.noteIndex].reason='Mengirim ke Roblox...'; updateJob(discordId,jobId,{songNotes:[...notes],robloxUploads:[...notes]}); const r=await uploadSingle({account,filePath:file,displayName:t.robloxDisplayName,description:description||account.defaultDescription,target,targetGroupId:t.dest}); notes[t.noteIndex]={part:t.out.part,groupId:t.dest||'',name:t.uiNoteName,status:'checking',assetId:null,pendingAssetId:r.assetId,operationId:r.operationId,reason:'Waiting for Roblox moderation / availability.',checkAttempts:0,createdAt:new Date().toISOString()}; }catch(e){ notes[t.noteIndex]={part:t.out.part,groupId:t.dest||'',name:t.uiNoteName,status:'failed',assetId:null,reason:e.message}; } updateJob(discordId,jobId,{songNotes:[...notes],robloxUploads:[...notes]}); })(); }); await Promise.all(promises); const hasC=notes.some(n=>n.status==='checking'), hasF=notes.some(n=>n.status==='failed'), hasA=notes.some(n=>n.status==='available'); const robloxStatus=hasC?'checking':(hasA&&hasF?'partial':hasA?'available':'failed'); updateJob(discordId,jobId,{robloxStatus,songNotes:notes,robloxUploads:notes}); await recheckJob(discordId,jobId,true); return getJob(discordId,jobId); }
...
app.post('/api/notes/:jobId/:part/mark-playable', requireAuth, (req,res)=>{ const job=getJob(req.user.id,req.params.jobId); if(!job) return res.status(404).json({error:'Job not found'}); const targetGroupId = req.body.groupId || null; const notes=(job.songNotes||[]).map(n=>{ const matchPart = Number(n.part)===Number(req.params.part); const matchGroup = !targetGroupId || String(n.groupId) === String(targetGroupId); if(matchPart && matchGroup){ n.status='available'; n.assetId=n.assetId||n.pendingAssetId||String(req.body.assetId||''); n.availableAt=new Date().toISOString(); n.reason=null; savePlayableNote(req.user.id,job,n,job.robloxDescription); } return n; }); const hasC=notes.some(n=>n.status==='checking'), hasF=notes.some(n=>n.status==='failed'), hasA=notes.some(n=>n.status==='available'); const robloxStatus=hasC?'checking':(hasA&&hasF?'partial':hasA?'available':'failed'); updateJob(req.user.id,job.id,{songNotes:notes,robloxStatus}); res.json(getJob(req.user.id,job.id)); });
app.post('/api/notes/:jobId/:part/mark-failed', requireAuth, (req,res)=>{ const job=getJob(req.user.id,req.params.jobId); if(!job) return res.status(404).json({error:'Job not found'}); const targetGroupId = req.body.groupId || null; const notes=(job.songNotes||[]).map(n=>{ const matchPart = Number(n.part)===Number(req.params.part); const matchGroup = !targetGroupId || String(n.groupId) === String(targetGroupId); if(matchPart && matchGroup){ n.status='failed'; n.reason=String(req.body.reason||'Marked failed manually'); } return n; }); const hasC=notes.some(n=>n.status==='checking'), hasF=notes.some(n=>n.status==='failed'), hasA=notes.some(n=>n.status==='available'); const robloxStatus=hasC?'checking':(hasA&&hasF?'partial':hasA?'available':'failed'); updateJob(req.user.id,job.id,{songNotes:notes,robloxStatus}); res.json(getJob(req.user.id,job.id)); });
