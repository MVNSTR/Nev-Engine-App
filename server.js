...
function usageSummary(discordId, user) {
  ...
  const convertLimit = user.dailyConvertLimit ?? DEFAULT_DAILY_CONVERT_LIMIT;
  const uploadLimit = user.dailyUploadLimit ?? DEFAULT_DAILY_UPLOAD_LIMIT;
  return {
    date: todayKey(),
...
function checkLimit(req,type){ const u=publicUser(req.user); const usage=getUsage(req.user.id)[todayKey()]||{convert:0,upload:0}; const used=usage[type]||0; const limit=type==='convert'?u.dailyConvertLimit:u.dailyUploadLimit; if(limit>=0 && used>=limit) return `${type} daily limit reached (${used}/${limit})`; return null; }
